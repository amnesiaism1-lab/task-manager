    ...
    <Modal
      ...
      title={
        <div className="flex items-center gap-2.5">
          <Badge variant="todo" size="sm">
            {issue?.issueType || issue?.type || 'Task'}
          </Badge>
          <span className="font-mono text-brand-400 font-bold">{issue?.key || 'Issue'}</span>
        </div>
      }
      description={
        issue ? `Project: ${issue.project?.name || 'Agile'} · Created ${formatDate(issue.createdAt)}` : ''
      }
      maxWidth="4xl"
    >
      {isLoading && !issue ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Right Sidebar: Attributes & Status (1 col) */}
          <div className="space-y-4 border-l border-border/80 pl-0 lg:pl-6">
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full bg-surface-surface text-text-primary text-xs rounded-lg px-2.5 py-1.5 border border-border"
              >
