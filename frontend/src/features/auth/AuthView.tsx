...
export const AuthView: React.FC = () => {
  ...
  // Initialize Google Identity Services with official Google Client ID
  useEffect(() => {
    const googleClientId = (import.meta as any).env?.VITE_GOOGLE_CLIENT_ID || '28140765256-rn44kbbuec2bp12jkqks4c55o5vnajcp.apps.googleusercontent.com';
  ...
  const launchGooglePopup = () => {
        ...
        const tokenClient = (window as any).google.accounts.oauth2.initTokenClient({
          ...
          callback: async (tokenResponse: any) => {
            if (tokenResponse?.error) {
              setError(`Google Sign-In was cancelled or failed: ${tokenResponse.error}`);
              return;
            }
            if (tokenResponse?.access_token) {
              try {
                ...
                setError(err.message || 'Google OAuth token verification failed on server');
              } finally {
                setGoogleLoading(false);
              }
            }
    ...
    setError('Google Sign-In is initializing. Please wait a moment and click again.');
  };
        ...
        {(authMode === 'login' || authMode === 'register') && (
          <>
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl bg-white hover:bg-slate-100 text-slate-800 text-xs font-semibold shadow-sm transition-all border border-slate-300 hover:shadow active:scale-[0.99] mb-5 group cursor-pointer disabled:opacity-50"
            >
