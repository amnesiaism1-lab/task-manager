    ...
    <div class="user-profile-modal-content space-y-6 text-slate-100">
      <div class="flex items-center gap-4 pb-4 border-b border-slate-800">
        ...
        <div>
          ...
          ${user.isSystemAdmin ? '<span class="inline-block mt-1 px-2 py-0.5 text-xs font-semibold rounded bg-purple-900/40 text-purple-300 border border-purple-700/50">⚡ System Admin</span>' : ''}
        </div>
      </div>
      ...
      <form id="form-change-password" class="space-y-3 bg-slate-900/60 p-4 rounded-xl border border-slate-800/80">
        ...
        <div class="form-group">
          ...
          <input id="prof-curr-pass" name="currentPassword" type="password" class="input-clean w-full" required />
        </div>
        <div class="form-group">
          <label for="prof-new-pass" class="text-xs text-slate-300 font-medium">New Password (8+ chars)</label>
          <input id="prof-new-pass" name="newPassword" type="password" minlength="8" class="input-clean w-full" required />
        </div>
        <button type="submit" class="button ghost btn-sm">Update Password</button>
      </form>

      <!-- Active Sessions (UC-AUTH-07) -->
      <div class="space-y-3">
          ...
          ${sessions.length ? sessions.map(s => `
            <div class="session-card-row p-3 rounded-lg bg-slate-900/80 border border-slate-800 flex items-center justify-between gap-3">
              <div class="session-info-main flex items-center gap-3">
                <span class="text-xl">${s.userAgent?.includes('Mobile') ? '📱' : '💻'}</span>
              ...
              <div>
                ...
                ` : `
                  <button type="button" class="button danger btn-sm btn-revoke-user-session" data-session-id="${s.id}">Revoke</button>
                `}
              </div>
            </div>
          ...
          ${(invites && invites.length) ? invites.map(inv => `
            <div class="p-3 rounded-lg bg-slate-900/80 border border-slate-800 flex items-center justify-between gap-3">
              ...
              <div class="flex items-center gap-2">
                <button type="button" class="button primary btn-sm btn-prof-accept-invite" data-inv-id="${escapeHtml(inv.id)}" data-org-name="${escapeHtml(inv.orgName || '')}">Accept</button>
                <button type="button" class="button ghost btn-sm btn-prof-decline-invite" data-inv-id="${escapeHtml(inv.id)}">Decline</button>
              </div>
            </div>
      ...
      <!-- Current Organization & Leave Org (UC-ORG-11) -->
      ${currentOrg ? `
        <div class="leave-org-card p-4 rounded-xl bg-red-950/20 border border-red-900/40">
          ...
          <button type="button" class="button danger btn-sm mt-3" id="btn-leave-workspace">
            Leave Organization
          </button>
        </div>
      ` : ''}
